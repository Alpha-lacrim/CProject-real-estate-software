...
void sign_up() {
    char username[30], name[20], last_name[40], email[100], phone_number[12], role[10];
    char pw[40], pw_1[40], pw_2[40], pw_cpy[40];
    char domain[5], input_BD[12], choice[5], owner_pw[40], input_pw[40];
    int birthdate[3];
    int i, j, z, count1, count2, count3;
    FILE *fp;
    ...
    while (1) {
        ...
        if (strcmp(role, "admin") == 0 || strcmp(role, "1") == 0) {
            ...
            if (strcmp(input_pw, owner_pw) == 0) {
                ...
                puts(input_pw);
                puts(owner_pw);
                printf("Wrong password, only owner can create admin\nPress any key to go back ...");
                getch();
                system("cls");
                continue;
            }
        ...
        while (1) { // name
            ...
            while (1) {
                ...
                if (birthdate[0] < 1900 || birthdate[0] > 2200) {
                    printf("Wrong input !\nPress any key to go back ...");
                    getch();
                    continue;
                } else {
        ...
        fprintf(fp,
                "ROLE: %s\nUSERNAME: %s\nNAME: %s\nLAST_NAME: %s\nEMAIL: %s\nBIRTHDATE: %d/%d/%d"
                "\nPHONE_NUMBER: %s\nPASSWORD: %s\n",
                info.role, info.username, info.name, info.last_name, info.email,
